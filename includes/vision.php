...
function wp_ai_workshop_demo_image_url_to_data_uri( $image_url ) {
	$response = wp_remote_get(
		...
		array(
			...
			// User-Agent, so send one when fetching the user-supplied image.
	...
	// Prefer the response Content-Type; fall back to detecting from the URL.
	...
	return 'data:' . $mime . ';base64,' . base64_encode( $body ); //phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode
}
